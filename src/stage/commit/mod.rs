    ...
    loop {
        tokio::select! {
            ...
            _ = tokio::time::sleep_until(report_timeout) => {
                ...
                let poll_duration = metrics.mean_scheduled_duration().as_secs_f64() * 1000f64;
                let poll_count = metrics.total_scheduled_count;
                pf_info!(id; "In the last minute: mean scheduled duration: {} ms, scheduled count: {}", poll_duration, poll_count);
