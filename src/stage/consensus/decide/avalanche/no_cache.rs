...
pub struct AvalancheNoCacheDecision {
    id: NodeId,
    ...
    fn handle_votes_helper(
        ...
        *confidence += 1;
        ...
        let (has_conflicts, count) = match avax_txn {
            AvalancheTxn::Grant { .. } | AvalancheTxn::Noop { .. } => (false, confidence_score),
            AvalancheTxn::Send {
                sender, in_utxo, ..
            } => {
        ...
        pf_trace!(self.id; "txn {} - is_accepted: {}, parents accepted: {}, have conflicts: {}, count: {}, pending txns: {}", txn_id, is_accepted, parents_accepted, has_conflicts, count, self.txn_dag.len());
    ...
    async fn handle_votes(&mut self, blk_id: u64) -> Result<(), CopycatError> {
        ...
        for idx in 0..txns.len() {
            let txn_hash = match txns[idx] {
                Some(hash) => hash,
                None => continue, // the txn is not valid, TODO: should never happen
            };
...
#[async_trait]
impl Decision for AvalancheNoCacheDecision {
    async fn new_tail(
        &mut self,
        src: NodeId,
    ...
    fn report(&mut self) {
        ...
        pf_info!(self.id; "working set size: txn_dag: {}", self.txn_dag.len());
