...
tput_ax.set_ylabel('Throughput (ktps.)', labelpad=4.2)
...
sched_ax.set_ylabel('Scheduling Delay (ms)', labelpad=2)
...
msg_ax.set_ylabel('Msg Late Chance (%)', labelpad=2)
...
cpu_ax.set_ylabel('CPU Utilization (%)', labelpad=0)
cpu_ax.set_ylim((0, 100))

cpu_ax.set_xlabel('# Validators')
cpu_ax.set_xticks(ecore2_df["num-nodes"])

plt.subplots_adjust(left=0.12, right=0.89, top=0.99, bottom=0.06, wspace=0, hspace=0.11)
# fig.tight_layout()
